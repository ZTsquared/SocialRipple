...
function handleSubmit(event){
  ...
  for(let i = 4; i <= 14; i++){
    if(event.target[i].checked){
      arrayToAdd.push(event.target[i].id)
    }
    setPreferences([...preferences, arrayToAdd])
  }
    ...
    <div className="mainMenu">
            ...
            <div>
                ...
                <form onSubmit={() => handleSubmit(event)} action="">
                    ...
                    <label htmlFor="organisation_input">organisation: <br />
                    <input value={organisation} name="organisation" onChange={handleChange} id="organisation" type="organisation" />
                    <br /><br />
                    </label> <br />
                    <label htmlFor="zipcode_input">zipcode: <br />
                    <input value={zipcode} name="zipcode" onChange={handleChange} id="zipcode" type="zipcode" />
                    <br /><br />
                    </label> <br />

                    {keywords?.map((keyword, index) => <div key={index}><input  id={keyword.id}type="radio" />{keyword.keyword}</div>)}
