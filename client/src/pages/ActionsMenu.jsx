...
export default function ActionsMenu() {
  ...
  const [displayActions, setDisplayActions] = useState({allVisible: false, group: false});
  ...
  function actionsToDisplayToggle(event){
    ...
    event.target.name === "display_actions_switch" && setDisplayActions({...displayActions, allVisible: !displayActions.allVisible})
  ...
  return (
    <div>
      ...
      { displayActions.allVisible &&
        ...
        <button name="group"      className="btn btn-success" onClick={actionsToDisplayToggle}>Group Actions</button>
        <button name="individual" className="btn btn-success" onClick={actionsToDisplayToggle}>Individual Actions</button>
      </div>}
      ...
      <div className="container">
        ...
        {displayActions.allVisible &&
        <div className="row">
          <h3>{displayActions.group ? "Group Actions" : "Individual Actions"}</h3>
          {actions.filter((a) =>(displayActions.group ? a.is_group : !a.is_group)).map(
            (action, index) =>
        ...
        <div className="row">
          <button name="display_actions_switch" onClick={actionsToDisplayToggle}className="btn btn-success">{displayActions.allVisible ? "back to recomended" : "Check out all upcomming actions"}</button>
        </div>
